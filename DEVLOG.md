...
## 2026-09-04 — M6–M10：按人工指示"循序渐进地全做"

人工授权沿候选清单连续推进；以下按里程碑记录，全部独立测试、独立提交。

### M6 工具调用对账（提交 54755e8）

- 机制：工具声明 `claim_pattern`（回复中其产物的引用）+ `result_id_pattern`（结果中的真实 ID）；最终回复与本轮审计比对，缺口触发一次纠正轮，对账事件入审计。
- 发现：`set(findall(...))` 把不可哈希 list 放入集合，真实演示当场崩溃——离线测试没抓住哈希性问题，真实数据形状补了位。另：M3 的跨进程重启测试在 M5/M6 两次编辑事故中立刻报警，回归测试价值实证。
- 验证：29 个测试；真实"工单 88"场景模型主动查证后如实回答。

### M7 RAG 关键词基线（提交 1395548）

- `aster/knowledge.py`：CJK 二元组 + ASCII 词、重叠计分、阈值 + Top-K；命中注入 system；`examples/kb.json` 为 Aster 自身 FAQ（真实内容）。
- 决策：embedding/向量检索明确推迟——关键词基线可离线验证，embedding 来源（MiniMax 平台或本地模型）待人工选定。
- 验证：37 个测试；真实演示两个知识库问题均由检索条目正确作答。

### M8 Web 渠道（提交 08c0b81）

- stdlib `ThreadingHTTPServer`（Web 框架决策继续推迟）：`/` 聊天页 + `/api/message`；与 Console 同一边界校验、同一 `build_runtime` 装配。
- 发现：第二个 Adapter 落地后规范消息契约无缺口语义——M1 边界设计经受住了复用检验。已知代价：无并发锁（GIL 下实验可接受）、无鉴权。
- 验证：44 个测试（服务器端口 0、按 room 隔离保证顺序无关）；curl 真实往返。

### M9 会话存储 SQLite 化（提交 0bde7ad）

- `storage.py` 迁移 stdlib `sqlite3`：原子性/写入锁交给 SQLite，手写 tmp+rename 退役；旧 JSON 文件大声失败（含回归测试）。
- 验证：45 个测试；真实两进程重启续号（echo #3），库内 6 条历史核对无误。

### M10 流式输出（提交 d226d58）

- `make_stream_reply`（SDK `messages.stream` 的 `text_stream`）+ `SessionStore.stream_handle`（边产边录、完成后落库、重复投递幂等）+ Web `/api/stream` SSE 与流式页面。
- 范围限制：仅纯聊天路径——工具轮次与 thinking 块交错，工具回复保持非流式；console 不提供流式。
- 验证：51 个测试；真实 `curl -N` 看到 11 个增量 delta 与 done 事件。

### M6–M10 遗留问题（汇总）

- 工具调用时机依赖模型自发性；对账兜底之外，意图级 tool_choice 需要意图分类层（等误判率数据）。
- RAG 为关键词基线；embedding 检索待来源选定。MCP 客户端暂缓（论证见 `PLAN.md` 开放问题）。
- Web 渠道无鉴权/并发锁；工单为内存假实现且不持久化；审计仅 stderr。

### 下一步建议

停止。剩余方向全部需要人工输入：真实渠道（平台账号/凭据）、embedding 来源、意图分类时机、部署形态与 Web 框架选型。

## 2026-09-04 — 代码流程优化（重构，零行为变更）

### 本次目标

人工指示"优化代码流程"。只消除真实重复与混乱流程，不改外部行为（CLI 参数、输出格式、存储格式、测试语义不变），不引入新抽象层。

### 实际完成

- `aster/agent.py`：`handle()` 改为 `stream_handle()` 的全文聚合（`"".join(...)`），删除两处各自维护的"查重 → 候选历史 → 落库 → 记轮次"重复流程；单一消息流从此只有一份实现；
- `aster/provider.py`：删除与 `make_chat_reply()` 语义重叠的 `chat_reply` 入口（测试改用后者）；`_chat` 拆出 `_apply_tool_round`（完整回传 + tool_result 追加）与 `_reconciliation`（对账与纠正请求）两个阶段函数，循环体恢复自顶向下可读；`make_chat_reply`/`make_stream_reply` 的知识检索重复收敛为 `_retrieved_context`；
- 测试同步：provider 测试改用 `make_chat_reply`，断言不变。

### 评估后不做的

- console.py 按"边界/CLI 装配"拆分两个模块：当前约 180 行且可读，拆分是纯搬运；
- web_channel 的 `_message`/`_stream` 错误路径去重：重复仅 4 行，抽取反而增加间接层。

### 执行过的验证

- `.venv` 51 个测试、裸 python 40+1 跳过，全部通过；
- 离线 Console 冒烟（SQLite 落盘 + 轮次推进）；
- 真实 API 冒烟：建单工具经重构后的 `_apply_tool_round` 成功执行，"工单 99"场景模型正确查证并如实回答，审计完整——重构触及的工具循环与对账路径均经真实验证。
