...
M1–M10 均已完成（2026-09-04）。人工验收经用户授权跳过；MCP 客户端按论证暂缓（见开放问题）。
...
- CJK 二元组 + ASCII 词检索，命中条目注入 system；`examples/kb.json` 为真实 FAQ。embedding/向量检索明确推迟到关键词质量不足时。提交 `1395548`。
...
- **MCP 客户端暂缓论证**：MCP 的价值在外部工具互通，当前 Aster 没有可对接的外部 MCP Server；官方 SDK 基于 asyncio，引入意味着进程模型转向。触发条件：出现第一个真实 MCP 工具源，或需要复用生态内现成工具时，再评估 client 接入（届时比较 asyncio 化与子进程桥接）。
- 工具调用时机仍依赖模型自发性（M6 对账可检出、可纠正）；意图级 tool_choice 强制需要意图分类层，等真实误判率数据再设计。

## 下一道人工确认门

M1–M10 已完成并停止。剩余方向全部需要人工输入或决策：真实消息渠道（需要平台账号/凭据）、embedding RAG（需要选定 embedding 来源）、意图分类层的引入时机、部署形态（含 Web 框架选型）。未获明确授权前不得自行开始。
